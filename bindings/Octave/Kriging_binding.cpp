...
void predict(int nlhs, mxArray** plhs, int nrhs, const mxArray** prhs) {
  ...
  auto* km = input.getObjectFromRef<Kriging>(0, "Kriging reference");
  const bool with_stdev = flag_output_compliance(input, 2, "with_stdev", output, 1);
  ...
  auto [y_pred, stderr_v, cov_m, mean_deriv_m, stderr_deriv_m]
      = km->predict(input.get<arma::mat>(1, "matrix"), with_stdev, with_cov, with_deriv);
...
void simulate(int nlhs, mxArray** plhs, int nrhs, const mxArray** prhs) {
  ...
  MxMapper output{"Output", nlhs, plhs, RequiresArg::Exactly{1}};
  auto* km = input.getObjectFromRef<Kriging>(0, "Kriging reference");
  auto result = km->simulate(input.get<int>(1, "nsim"), input.get<int>(2, "seed"), input.get<arma::mat>(3, "X_n"), input.get<bool>(4, "will_update"));
