% predictIterative: matrix-free conjugate-gradient prediction, mirroring the
% C++/R/Julia/Python predictIterative test suites' test function and fixed-theta
% setup (see docs/math/PredictIterative.md for why theta is fixed: a free BFGS
% fit is known to drift theta toward a near-singular correlation matrix on
% this noise-free deterministic function, unrelated to predictIterative itself).
...
% "Theta should be given" before ever reaching predictIterative.
...
m_only = k.predictIterative(Xt);
assert(length(m_only) == size(Xt, 1));

% interpolates the training data
[m_train, s_train] = k.predictIterative(X, true);
...
threw = false;
try
    k.predictIterative(Xt, false, int32(-1));
catch err
    threw = true;
end
assert(threw);
...
[m_pc, s_pc] = k.predictIterative(Xt, true, int32(0), 1e-8, true, int32(20));
...
threw = false;
try
    k.predictIterative(Xt, false, int32(0), 1e-8, true, int32(-1));
catch err
    threw = true;
end
assert(threw);
...
disp("KrigingPredictIterative_test: all assertions passed");
