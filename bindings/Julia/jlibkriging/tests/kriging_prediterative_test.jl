...
# f2d matches the C++/R/Python predictIterative test suites' test function.
...
# docs/math/PredictIterative.md / KrigingNystromTest.cpp for the same issue),
# which would make predict/predictIterative's agreement noisy rather than a clean
# correctness signal.
make_fixed_theta_model(y, X; theta_val::Float64=0.3) =
    Kriging(y, X, "matern5_2"; optim="none", theta=[theta_val, theta_val], sigma2=1.0)

@testset "Kriging predictIterative" begin
    ...
    @testset "interpolates the training data" begin
        ...
        p = predictIterative(k, X; return_stdev=true)
        sdy = std(y)
    ...
    @testset "rejects a negative max_iter" begin
        ...
        @test_throws ArgumentError predictIterative(k, Xt; max_iter=-1)
    end
    ...
    @testset "Nystrom-preconditioned CG matches exact predict" begin
        ...
        p_exact = predict(k, Xt; return_stdev=true)
        p_pc = predictIterative(k, Xt; return_stdev=true, use_nystrom_precond=true, precond_rank=20)
    ...
    @testset "rejects a negative precond_rank" begin
        ...
        @test_throws ArgumentError predictIterative(k, Xt; use_nystrom_precond=true, precond_rank=-1)
    end
end
