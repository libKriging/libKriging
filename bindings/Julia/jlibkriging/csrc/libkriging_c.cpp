...
#define CATCH_RETURN_NULL \
  catch (const std::exception& e) { g_last_error = e.what(); return nullptr; }

#define CATCH_RETURN_NAN \
  catch (const std::exception& e) { g_last_error = e.what(); return std::numeric_limits<double>::quiet_NaN(); }
...
int lk_linear_regression_fit(void* ptr,
                              const double* y, int n,
                              const double* X, int nX, int d) {
  try {
...
int lk_linear_regression_predict(void* ptr,
                                  const double* X, int m, int d,
...
void* lk_kriging_new_fit(const double* y, int n,
                          const double* X, int nX, int d,
                          const char* kernel,
                          const char* regmodel,
                          int normalize,
                          ...
                          const double* sigma2,
                          int is_sigma2_estim,
                          const double* theta, int theta_n,
                          int is_theta_estim,
                          const double* beta, int beta_n,
                          int is_beta_estim) {
  try {
    arma::vec y_v(const_cast<double*>(y), n, false, true);
    arma::mat X_m(const_cast<double*>(X), nX, d, false, true);
...
int lk_kriging_update(void* ptr,
                       const double* y_u, int n,
                       const double* X_u, int nX, int d,
                       int refit) {
  try {
    auto* k = static_cast<Kriging*>(ptr);
...
int lk_kriging_update_simulate(void* ptr,
                                const double* y_u, int n,
                                const double* X_u, int nX, int d,
                                double* sim_out, int* nsim_out, int* m_out) {
  try {
    auto* k = static_cast<Kriging*>(ptr);
    arma::vec y_v(const_cast<double*>(y_u), n, false, true);
    arma::mat X_m(const_cast<double*>(X_u), nX, d, false, true);
    arma::mat sim = k->update_simulate(y_v, X_m);
    if (nsim_out) *nsim_out = static_cast<int>(sim.n_cols);
    if (m_out) *m_out = static_cast<int>(sim.n_rows);
    if (sim_out)
      std::memcpy(sim_out, sim.memptr(), sim.n_elem * sizeof(double));
    return 0;
  } CATCH_RETURN
...
int lk_kriging_log_likelihood_fun(void* ptr,
                                   const double* theta, int theta_n,
                                   int return_grad, int return_hess,
                                   double* ll_out,
                                   double* grad_out,
                                   double* hess_out) {
  try {
...
int lk_kriging_leave_one_out_vec(void* ptr,
                                  const double* theta, int theta_n,
                                  double* yhat_out, double* stderr_out) {
  try {
    auto* k = static_cast<Kriging*>(ptr);
    arma::vec theta_v(const_cast<double*>(theta), theta_n, false, true);
...
int lk_kriging_cov_mat(void* ptr,
                        const double* X1, int n1, int d1,
                        const double* X2, int n2, int d2,
                        double* cov_out) {
  try {
    auto* k = static_cast<Kriging*>(ptr);
...
int lk_kriging_get_X(void* ptr, double* out, int* n, int* d) {
  try {
    ...
    if (n) *n = static_cast<int>(v.n_rows);
    if (d) *d = static_cast<int>(v.n_cols);
    if (out)
...
int lk_kriging_get_centerX(void* ptr, double* out, int* d) {
  try {
    ...
    if (d) *d = static_cast<int>(v.n_elem);
    if (out)
      std::memcpy(out, v.memptr(), v.n_elem * sizeof(double));
    return 0;
  } CATCH_RETURN
...
int lk_kriging_get_y(void* ptr, double* out, int* n) {
  try {
    ...
    if (n) *n = static_cast<int>(v.n_elem);
    if (out)
      std::memcpy(out, v.memptr(), v.n_elem * sizeof(double));
    return 0;
  } CATCH_RETURN
...
void* lk_nugget_kriging_new_fit(const double* y, int n,
                                 const double* X, int nX, int d,
                                 const char* kernel,
                                 const char* regmodel,
                                 int normalize,
                                 const char* optim,
                                 const char* objective,
                                 const double* sigma2, int sigma2_n,
                                 int is_sigma2_estim,
                                 const double* theta, int theta_n,
                                 int is_theta_estim,
                                 const double* beta, int beta_n,
                                 int is_beta_estim,
                                 const double* nugget, int nugget_n,
                                 int is_nugget_estim) {
  try {
    arma::vec y_v(const_cast<double*>(y), n, false, true);
    arma::mat X_m(const_cast<double*>(X), nX, d, false, true);
    ...
    return new NuggetKriging(y_v, X_m,
                             kernel ? kernel : "matern3_2",
                             Trend::fromString(regmodel ? regmodel : "constant"),
                             normalize != 0,
                             optim ? optim : "BFGS",
...
int lk_nugget_kriging_update(void* ptr,
                              const double* y_u, int n,
                              const double* X_u, int nX, int d,
                              int refit) {
  try {
...
void* lk_noise_kriging_new_fit(const double* y, int n,
                                const double* noise, int noise_n,
                                const double* X, int nX, int d,
                                const char* kernel,
                                const char* regmodel,
                                int normalize,
  ...
  try {
    ...
    return new NoiseKriging(y_v, noise_v, X_m,
                            kernel ? kernel : "matern3_2",
                            Trend::fromString(regmodel ? regmodel : "constant"),
                            normalize != 0,
                            optim ? optim : "BFGS",
...
int lk_noise_kriging_simulate(void* ptr,
                               int nsim, int seed,
                               const double* X_n, int m, int d,
                               const double* with_noise, int noise_n,
                               int will_update,
                               double* sim_out) {
  try {
    auto* k = static_cast<NoiseKriging*>(ptr);
...
int lk_noise_kriging_update(void* ptr,
                             const double* y_u, int n,
                             const double* noise_u, int noise_n,
                             const double* X_u, int nX, int d,
                             int refit) {
  try {
    auto* k = static_cast<NoiseKriging*>(ptr);
