...
ll = Vectorize(function(x) logLikelihoodFun(r,x)$logLikelihoodFun)
...
ll = function(X) {if (!is.matrix(X)) X = matrix(X,ncol=2);
                  ...
                  apply(X,1,
                    function(x) {
                      ...
                      y=-logLikelihoodFun(r,matrix(unlist(x),ncol=2))$logLikelihoodFun
                      #print(y);
                      y})}
...
apply(X,1,
      function(x) {
        # print(dim(x))
        #print(matrix(unlist(x),ncol=2));
        -logLikelihoodFun(r,matrix(unlist(x),ncol=2))$logLikelihoodFun
        #print(y);
        })}
#DiceView::contourview(ll,xlim=c(0.01,2),ylim=c(0.01,2))
x1=seq(0.001,2,,51)
