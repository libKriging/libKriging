...
m1 <- rgasp(design = input, response = output,
  # Default parameters:
  trend=matrix(1,length(response),1),zero.mean="No",nugget=0,
  nugget.est=F,range.par=NA,
  method='post_mode',prior_choice='ref_approx',
  a=0.2, b=1/(length(response))^{1/dim(as.matrix(design))[2]}*(a+dim(as.matrix(design))[2]),
