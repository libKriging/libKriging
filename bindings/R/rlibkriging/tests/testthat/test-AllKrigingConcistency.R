...
for (i in 1:length(X_n)) {
    ...
    mean(lus_no[i,]),",",sd(lus_no[i,])," != ",mean(lus_nu[i,]),",",sd(lus_nu[i,])),
        expect_true(suppressWarnings(ks.test(lus_no[i,],lus_nu[i,])$p.value) > 0.01)) # just check that it is not clearly wrong
}
...
for (i in 1:length(X_n)) {
    ...
    mean(lus_no[i,]),",",sd(lus_no[i,])," != ",mean(lus_nu[i,]),",",sd(lus_nu[i,])),
        expect_true(suppressWarnings(ks.test(lus_no[i,],lus_nu[i,])$p.value) > 0.00001)) # just check that it is not clearly wrong
}
