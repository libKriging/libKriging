...
test_that(desc="logLik is the same that DiceKriging one", 
         expect_true(abs(ordinary_kriging_loglikelihood(r,x)-DiceKriging::logLikFun(x,k,xenv)) < precision))

test_that(desc="logLik Grad is the same that DiceKriging one", 
         expect_true(abs(ordinary_kriging_loglikelihoodgrad(r,x)-DiceKriging::logLikGrad(x,k,xenv)) < precision))
