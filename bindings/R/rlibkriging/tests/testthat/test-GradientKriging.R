...
context("Gradient-enhanced kriging (grady)")
...
test_that("grady interpolates values and gradients", {
  d <- make_design(20)
  k <- Kriging(d$y, d$X, "gauss", grady = d$dy)
...
test_that("fit(..., grady=) clears gradient observations on a later fit without grady", {
  d <- make_design(20)
  k <- Kriging("gauss")
  fit(k, d$y, d$X, grady = d$dy)
...
test_that("grady rejects a non-differentiable kernel", {
  d <- make_design(10)
  expect_error(Kriging(d$y, d$X, "exp", grady = d$dy))
})

test_that("grady rejects a wrongly shaped matrix", {
  d <- make_design(10)
  expect_error(Kriging(d$y, d$X, "gauss", grady = d$dy[, 1, drop = FALSE]))
})
