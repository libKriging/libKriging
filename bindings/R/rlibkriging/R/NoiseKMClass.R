...
## Register the S3 class "NoiseKriging" to define the class of the @NoiseKriging
## slot in a `NoiseKM` object
setOldClass("NoiseKriging")

#if (requireNamespace("DiceKriging", quietly = TRUE))
## *****************************************************************************
#' @title S4 class for NoiseKriging Models Extending the \code{"km"} Class
#' 
...
#'     \pkg{DiceKriging} package. The creator \code{\link{NoiseKM}}.
#' 
setClass("NoiseKM", slots = c("NoiseKriging" = "NoiseKriging"), contains = "km")
...
#' @param ... Ignored.
#'
#' @return A NoiseKM object. See \bold{Details}.
#'
...
NoiseKM <- function(formula = ~1, design, response,
    ...
    r <- rlibkriging::NoiseKriging(y = response, noise = noise.var, X = design, kernel = covtype,
                              regmodel = formula,
                              normalize = FALSE,
    ...
    return(as.km.NoiseKriging(r, .call = match.call()))
}

## *****************************************************************************
## 'predict' S4 method We no longer export 'predict.NoiseKM'
## *****************************************************************************

predict.NoiseKM <- function(object, newdata, type = "UK",
                       se.compute = TRUE,
                       cov.compute = FALSE,
                       light.return = TRUE,
...
#' 
#' @importFrom stats qt
#' @method predict NoiseKM
#' @exportMethod predict
#' @aliases predict,NoiseKM-method
#'
#' @examples
#' ## a 16-points factorial design, and the corresponding response
#' d <- 2; n <- 16
...
#' 
setMethod("predict", "NoiseKM", predict.NoiseKM)


## *****************************************************************************
## 'simulate' S4 method We no longer export 'simulate.NoiseKM'
## *****************************************************************************

simulate.NoiseKM <- function(object, nsim = 1, seed = NULL, newdata,
  ...
  return(simulate.NoiseKriging(object = object@NoiseKriging,
...
#' @export
#' @aliases simulate,NoiseKM-method
...
#' 
setMethod("simulate", "NoiseKM", simulate.NoiseKM)

## *****************************************************************************
## 'update' S4 method We no longer export 'update.NoiseKM'
## *****************************************************************************
...
#' \code{"NoiseKM"}, this would have been inherited from the class
#' \code{"km"}. The dedicated method is expected to run faster.  A
#' comparison can be made by coercing a \code{NoiseKM} object to a
#' \code{km} object with \code{\link{as.km}} before calling
#' \code{update}.
#'
...
#' @param ... Ignored.
#'
#' @return The updated \code{NoiseKM} object.
#' 
#' @seealso \code{\link{as.km}} to coerce a \code{NoiseKM} object to the
#'     class \code{"km"}.
#'
#' @export
#' @exportMethod update
#' @aliases update,NoiseKM-method
#' 
#' @examples
#' f <- function(x) 1 - 1 / 2 * (sin(12 * x) / (1 + x) + 2 * cos(7 * x) * x^5 + 0.7)
#' plot(f)
...
#' 
setMethod("update", "NoiseKM", update.NoiseKM)
