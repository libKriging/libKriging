...
#  print(n)
  ...
  times$cpp[i] = system.time(
    ...
    try({for (t in 1:N) ll2x <- kriging_logLikelihoodFun(r,rep(theta,ncol(X)))}) # Loop should be done inside c++, not from R...
  )[1]
...
#  print(n)
  ...
  r <- kriging(y,X,"gauss","constant",FALSE,"none","LL",
             ...
             theta=matrix(k@covariance@range.val,ncol=d)))
  times$cpp[i] = system.time(
    try({for (t in 1:N) loo2x <- kriging_leaveOneOutFun(r,rep(theta,ncol(X)))}) # Loop should be done inside c++, not from R...
  )[1]
...
#  print(n)
  ...
  times$cpp[i] = system.time(
    # try({gll2x <- bench_LogLikGrad(N,r,rep(theta,ncol(X)))})
    try({for (t in 1:N) gll2x <- kriging_leaveOneOutGrad(r,rep(theta,ncol(X)))}) # Loop should be done inside c++, not from R...
  )[1]
...
#  print(n)
  ...
  ll_cpp <- kriging_logLikelihoodFun(r, kriging_model(r)$theta)
  e <- new.env()
  ...
  if (abs(ll_cpp - DiceKriging::logLikFun(param=as.numeric(kriging_model(r)$theta),model=k))/ll_cpp>.1)
    warning("LL function is not the same bw DiceKriging/libKriging: ",kriging_logLikelihoodFun(r,kriging_model(r)$theta)," vs. ",DiceKriging::logLikFun(param=as.numeric(kriging_model(r)$theta),model=k))  
...
for (i in 1:length(logn)) {
 ...
 # print(n)
  ...
  ll_cpp <- kriging_leaveOneOutFun(r, kriging_model(r)$theta)
  e <- new.env()
  ll_R <- DiceKriging::leaveOneOutFun(k@covariance@range.val, k, e)
  
  gll_cpp <- kriging_leaveOneOutGrad(r, kriging_model(r)$theta)
  gll_R <- DiceKriging::leaveOneOutGrad(k@covariance@range.val, k, e)
  
  if (abs(ll_cpp - DiceKriging::leaveOneOutFun(param=as.numeric(kriging_model(r)$theta),model=k))/ll_cpp>.1)
    warning("LOO function is not the same bw DiceKriging/libKriging: ",kriging_leaveOneOutFun(r,kriging_model(r)$theta)," vs. ",DiceKriging::leaveOneOutFun(param=as.numeric(kriging_model(r)$theta),model=k))  
...
#  print(n)
  ...
  times$cpp[i] = system.time(
    try({for (t in 1:N) p2 <- kriging_predict(r,newX,TRUE,FALSE)}) # Loop should be done inside c++, not from R...
  )[1]
...
#  print(n)
  ...
  r <- kriging(y, X,"gauss", "constant", FALSE,"none", "LL",
               parameters=list(sigma2=k@covariance@sd2,theta=matrix(k@parinit,ncol=d)))
  times$cpp[i] = system.time(
    try({for (t in 1:N) s2 <- kriging_simulate(r,10,newX)}) # Loop should be done inside c++, not from R...
  )[1]
