...
This report is intended to display a (quite) full benchmark of armadillo/libKriging imlpementation and methods versus R/DiceKriging ones.
...
#  print(n)
  ...
  r <- kriging(y,X,"gauss","constant",FALSE,"none","LL",
             ...
             theta=matrix(k@covariance@range.val,ncol=d)))
  times$cpp[i] = system.time(
    try({for (t in 1:N) loo2x <- kriging_leaveOneOut(r,rep(theta,ncol(X)))}) # Loop should be done inside c++, not from R...
  )[1]
...
#  print(n)
  ...
  if (abs(ll_cpp - DiceKriging::logLikFun(param=as.numeric(kriging_model(r)$theta),model=k))/ll_cpp>.1)
    warning("LL function is not the same bw DiceKriging/libKriging: ",kriging_logLikelihood(r,kriging_model(r)$theta)," vs. ",DiceKriging::logLikFun(param=as.numeric(kriging_model(r)$theta),model=k))  
...
#  print(n)
  ...
  ll_cpp <- kriging_leaveOneOut(r, kriging_model(r)$theta)
  e <- new.env()
  ll_R <- DiceKriging::logLikFun(k@covariance@range.val, k, e)
  ...
  if (abs(ll_cpp - DiceKriging::logLikFun(param=as.numeric(kriging_model(r)$theta),model=k))/ll_cpp>.1)
    warning("LOO function is not the same bw DiceKriging/libKriging: ",kriging_logLikelihood(r,kriging_model(r)$theta)," vs. ",DiceKriging::logLikFun(param=as.numeric(kriging_model(r)$theta),model=k))  
...
#  print(n)
  ...
  if (abs(p$mean-p2$mean)>1E-3) warning("predict is not identical bw C++/R")
}
...
#  print(n)
  ...
  k <- DiceKriging::km(design=X, response=y, covtype = "gauss", control=list(trace=F))
  times$R[i] = system.time(
    try({for (t in 1:N) s <- DiceKriging::simulate(k,newdata = newX, nsim=10,checkNames=FALSE)})
  )[1]
