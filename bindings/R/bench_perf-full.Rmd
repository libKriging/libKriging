...
#  print(n)
  ...
  ll_cpp <- logLikelihood(r, as.list(r)$theta)$logLikelihood[1]
  e <- new.env()
  ll_R <- DiceKriging::logLikFun(k@covariance@range.val, k, e)
  
  gll_cpp <- logLikelihood(r, as.list(r)$theta,grad=T)$logLikelihoodGrad
  ...
  if (abs(ll_cpp - DiceKriging::logLikFun(param=as.numeric(as.list(r)$theta),model=k))/ll_cpp>.1)
    warning("LL function is not the same bw DiceKriging/libKriging: ",logLikelihood(r,as.list(r)$theta)$logLikelihood[1]," vs. ",DiceKriging::logLikFun(param=as.numeric(as.list(r)$theta),model=k))  
...
for (i in 1:length(logn)) {
 ...
 # print(n)
  ...
  ll_cpp <- leaveOneOut(r, as.list(r)$theta)$leaveOneOut[1]
  e <- new.env()
  ll_R <- DiceKriging::leaveOneOutFun(k@covariance@range.val, k, e)
  
  gll_cpp <- leaveOneOut(r, as.list(r)$theta,grad=T)$leaveOneOutGrad
  ...
  if (abs(ll_cpp - DiceKriging::leaveOneOutFun(param=as.numeric(as.list(r)$theta),model=k))/ll_cpp>.1)
    warning("LOO function is not the same bw DiceKriging/libKriging: ",leaveOneOut(r,as.list(r)$theta)$leaveOneOut[1]," vs. ",DiceKriging::leaveOneOutFun(param=as.numeric(as.list(r)$theta),model=k))  
...
#  print(n)
  ...
  ll_cpp <- logLikelihoodFun(r, as.list(r)$theta)
  e <- new.env()
  ll_R <- DiceKriging::logLikFun(k@covariance@range.val, k, e)
  
  gll_cpp <- logLikelihoodGrad(r, as.list(r)$theta)
  gll_R <- DiceKriging::logLikGrad(k@covariance@range.val, k, e)
