...
#  print(n)
  ...
  times$cpp[i] = system.time(
    ...
    try({for (t in 1:N) ll2x <- rlibkriging::logLikelihoodFun(r,rep(theta,ncol(X)))$logLikelihoodFun[1]}) # Loop should be done inside c++, not from R...
  )[1]
...
plot_times(times,N,"logLikelihoodFun Grad")
```
...
#### logLikelihoodFun / BFGS

```{r logLikelihoodFun-BFGS}
times <- list(R=rep(NA, length(logn)), cpp=rep(NA, length(logn)))
...
#  print(n)
  ...
  ll_cpp <- logLikelihoodFun(r, t(as.list(r)$theta))$logLikelihoodFun[1]
  e <- new.env()
  ...
  if (abs(ll_cpp - DiceKriging::logLikFun(param=as.numeric(as.list(r)$theta),model=k))/ll_cpp>.1)
    warning("LL function is not the same bw DiceKriging/libKriging: ",logLikelihoodFun(r,t(as.list(r)$theta))$logLikelihoodFun[1]," vs. ",DiceKriging::logLikFun(param=as.numeric(as.list(r)$theta),model=k))  
...
plot_times(times,N,"fit logLikelihoodFun / BFGS")
```
...
#### logLikelihoodFun / Newton

```{r logLikelihoodFun-Newton}
times <- list(R=rep(NA, length(logn)), cpp=rep(NA, length(logn)))
...
plot_times(times,N,"fit logLikelihoodFun / Newton")
```
