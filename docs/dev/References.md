...
* Xiong, Y., Chen, W., Apley, D. and Ding, X. (2007). A Non-Stationary Covariance-Based Kriging Method for Metamodelling in Engineering Design (`knots`, piecewise-linear warp — also implemented in DiceKriging's `knots` argument). *International Journal for Numerical Methods in Engineering*, 71(6), 733–756. doi:10.1002/nme.1969
* Archer, N. P. and Wang, S. (1993). Application of the Back Propagation Neural Network Algorithm with Monotonicity Constraints for Two-Group Classification Problems (`neural_mono`, positive-weight monotone network). *Decision Sciences*, 24(1), 60–75. doi:10.1111/j.1540-5915.1993.tb00464.x
...
* Wilson, A. G., Hu, Z., Salakhutdinov, R. and Xing, E. P. (2016). Deep Kernel Learning (`mlp_joint`/`MLPKriging`, joint cross-variable feature map). *Proceedings of the 19th AISTATS*, PMLR 51, 370–378.
...
* Garrido-Merchán, E. C. and Hernández-Lobato, D. (2020). Dealing with categorical and integer-valued variables in Bayesian Optimization with Gaussian processes (`categorical`/`ordinal` encodings). *Neurocomputing*, 380, 20–35. doi:10.1016/j.neucom.2019.11.004
