...
static void test_simulate_mixed() {
  ...
  WarpKriging model({"affine", "categorical(2,2)"},
                    "gauss");
  model.fit(y, X, "constant", false, "Adam", "LL",
            {{"max_iter_adam", "200"}});
...
static void test_from_string_roundtrip() {
  ...
  // Invalid strings
  bool caught = false;
  try { WarpSpec::from_string("foobar"); } catch (...) { caught = true; }
  assert(caught);
  std::cout << "  \"foobar\" -> exception  OK" << std::endl;

  caught = false;
  try { WarpSpec::from_string("categorical"); } catch (...) { caught = true; }
  assert(caught);
...
static void test_warping_strings_accessor() {
  ...
  for (arma::uword i = 0; i < 10; ++i) X(i, 1) = i % 3;
  X.col(2).fill(0.0);  // ordinal levels
  for (arma::uword i = 0; i < 10; ++i) X(i, 2) = i % 4;
...
static void test_mlp_joint() {
  ...
  // --- 16a: 1D function ---
  {
    ...
    model.fit(y, X_mat, "constant", true, "Adam", "LL",
              {{"max_iter_adam", "300"}});
  ...
  // --- 16b: Branin 2D (exact equivalent of old NeuralKernelKriging test) ---
  {
    ...
    std::cout << "  [2D] Predicted mean range:  ["
              << arma::min(mean) << ", " << arma::max(mean) << "]" << std::endl;
    std::cout << "  [2D] Predicted stdev range: ["
