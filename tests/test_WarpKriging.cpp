...
static double check_warp_grad(IWarp& w, const arma::vec& x) {
  ...
  if (params.empty()) return 0.0;  // no params
  ...
  double rel = arma::norm(grad_analytic - grad_numeric)
...
static void test_warp_functions() {
  ...
  // -- NeuralMono --
  {
    ...
    for (arma::uword i = 1; i < out.n_rows; ++i)
      if (out(i, 0) < out(i - 1, 0) - 1e-10) mono = false;
  ...
  // -- MLP (unconstrained, multi-dim output) --
  {
    ...
    assert(out.is_finite());
    std::cout << "  MLP output shape:       "
              << out.n_rows << " x " << out.n_cols << std::endl;
  ...
  // -- Ordinal --
  {
    ...
    for (arma::uword l = 1; l < 4; ++l) {
      ...
      for (arma::uword i = 0; i < x_ord.n_elem; ++i) {
        if (std::round(x_ord(i)) == l) z_l = out(i, 0);
        if (std::round(x_ord(i)) == l - 1) z_lm1 = out(i, 0);
      }
      if (z_l <= z_lm1) ordered = false;
    }
...
static void test_continuous_kumaraswamy() {
  ...
  WarpKriging model({WarpSpec::kumaraswamy()}, "gauss");
  model.fit(y, X_mat, "constant", false, "Adam", "LL",
            {{"max_iter_adam", "200"}});
  ...
  arma::mat xp(50, 1); xp.col(0) = x_pred;
  auto [mean, stdev, __] = model.predict(xp, true, false);
  arma::vec ytrue(50);
  for (arma::uword i = 0; i < 50; ++i) ytrue(i) = f1d(x_pred(i));
...
static void test_categorical_only() {
  ...
  WarpKriging model({WarpSpec::categorical(3, 2)}, "gauss");
  ...
  arma::mat X_test(3, 1);
  X_test(0, 0) = 0.0; X_test(1, 0) = 1.0; X_test(2, 0) = 2.0;
  auto [mean, stdev, _] = model.predict(X_test, true, false);

  std::cout << "  Predictions per level:\n";
  for (int l = 0; l < 3; ++l)
    std::cout << "    level " << l << ": mean=" << mean(l)
              << ", stdev=" << stdev(l) << ", true_mu=" << mu[l] << std::endl;
...
static void test_mixed() {
  ...
  WarpKriging model({
    WarpSpec::kumaraswamy(),       // x0: continuous
    WarpSpec::categorical(3, 2)    // x1: 3-level categorical
  }, "matern5_2");
...
static void test_ordinal() {
  ...
  for (arma::uword i = 0; i < n; ++i) {
    ...
    X(i, 0) = static_cast<double>(level);
    y(i) = static_cast<double>(level * level)
           + 0.1 * arma::randn<arma::vec>(1)(0);
  }

  WarpKriging model({WarpSpec::ordinal(L)}, "gauss");
  model.fit(y, X, "constant", false, "Adam", "LL",
            {{"max_iter_adam", "200"}});
  ...
  arma::mat X_test(L, 1);
  for (arma::uword l = 0; l < L; ++l) X_test(l, 0) = static_cast<double>(l);
  auto [mean, stdev, _] = model.predict(X_test, true, false);

  std::cout << "  Predictions:\n";
  for (arma::uword l = 0; l < L; ++l)
    std::cout << "    level " << l << ": pred=" << mean(l)
              << ", true=" << l * l << std::endl;
...
static void test_neural_mono() {
  ...
  WarpKriging model({WarpSpec::neural_mono(8)}, "gauss");
  model.fit(y, X_mat, "constant", false, "Adam", "LL",
            {{"max_iter_adam", "200"}});
...
static void test_mlp_warp() {
  ...
  WarpKriging model1d({WarpSpec::mlp({16, 8}, 3, "selu")}, "gauss");
  model1d.fit(y, X_mat, "constant", false, "Adam", "LL",
              {{"max_iter_adam", "300"}});
  ...
  WarpKriging model_mix({
    WarpSpec::mlp({16, 8}, 2, "tanh"),   // x0: MLP → ℝ²
    WarpSpec::categorical(3, 2)           // x1: 3 cats → ℝ²
  }, "matern5_2");

  model_mix.fit(y_mix, X_mix, "constant", false, "Adam", "LL",
                {{"max_iter_adam", "300"}});
...
static void test_simulate_mixed() {
  ...
  WarpKriging model({WarpSpec::affine(), WarpSpec::categorical(2, 2)},
                    "gauss");
  model.fit(y, X, "constant", false, "Adam", "LL",
            {{"max_iter_adam", "200"}});
...
static void test_update() {
  ...
  WarpKriging model({WarpSpec::none(), WarpSpec::categorical(2, 1)}, "gauss");
  model.fit(y0, X0, "constant", false, "Adam", "LL",
            {{"max_iter_adam", "100"}});
...
static void test_branin_2d_mlp() {
  ...
  std::cout << "  Predicted mean range:  ["
            << arma::min(mean_test) << ", " << arma::max(mean_test) << "]" << std::endl;
  std::cout << "  Predicted stdev range: ["
            << arma::min(stdev_test) << ", " << arma::max(stdev_test) << "]" << std::endl;
...
static void test_none_vs_mlp() {
  ...
  WarpKriging model_none({WarpSpec::none()}, "gauss");
  model_none.fit(y, X_mat, "constant", false, "Adam", "LL",
                 {{"max_iter_adam", "200"}});
  ...
  // Model B: MLP warping
  WarpKriging model_mlp({WarpSpec::mlp({16, 8}, 2, "selu")}, "gauss");
  model_mlp.fit(y, X_mat, "constant", false, "Adam", "LL",
                {{"max_iter_adam", "300"}});
  ...
  WarpKriging model_kuma({WarpSpec::kumaraswamy()}, "gauss");
  model_kuma.fit(y, X_mat, "constant", false, "Adam", "LL",
                 {{"max_iter_adam", "200"}});
...
static void test_loglikelihood_fun() {
  ...
  WarpKriging model({WarpSpec::affine()}, "gauss");
  model.fit(y, X_mat, "constant", false, "Adam", "LL",
            {{"max_iter_adam", "100"}});
