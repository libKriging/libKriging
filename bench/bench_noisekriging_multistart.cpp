...
void generate_branin_data(arma::mat& X, arma::vec& y, arma::vec& noise, int n) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    noise(i) = 0.01;  // Known noise variance
  }
  // Add noise to y
  y += arma::sqrt(noise) % arma::randn(n);
}
...
int main(int argc, char* argv[]) {
  ...
  arma::vec y;
  arma::vec noise;
  generate_branin_data(X, y, noise, n);
  
  std::cout << "NoiseKriging Multistart Benchmark" << std::endl;
  std::cout << "Dataset: n=" << n << " points, d=2 dimensions (with known noise)" << std::endl;
  ...
  for (size_t m = 0; m < methods.size(); m++) {
    ...
    for (int r = 0; r < n_runs; r++) {
      NoiseKriging km("gauss");
      auto start = std::chrono::high_resolution_clock::now();
      km.fit(y, noise, X, Trend::RegressionModel::Constant, false, methods[m], "LL");
