...
void benchmark_configuration(arma::uword n_train, arma::uword d, int n_iterations) {
  ...
  const arma::uword n_pred_stdev = 10;
  arma::mat X_pred(n_pred, d, arma::fill::randu);
  arma::mat X_pred_stdev = X_pred.head_rows(n_pred_stdev);

  Kriging k(y_train, X_train, "gauss", Trend::RegressionModel::Constant, false, "BFGS", "LL");

  for (int iter = 0; iter < n_iterations; ++iter) {
    {
      auto t0 = std::chrono::high_resolution_clock::now();
      k.predict(X_pred, true, false, false);
