...
void benchmark_configuration(arma::uword n_train, arma::uword d, int n_iterations) {
  ...
  // Prepare test data with noise variance
  arma::mat X_train(n_train, d, arma::fill::randu);
  arma::colvec y_train(n_train);
  arma::colvec noise_train(n_train, arma::fill::value(0.01));  // Small noise variance
  ...
  arma::colvec noise_update(n_update, arma::fill::value(0.01));
  ...
  for (int iter = 0; iter < n_iterations; ++iter) {
    ...
    // Benchmark SIMULATE
    {
      ...
      int nsim = 100;
      int seed = 123 + iter;
      arma::colvec noise_pred(n_pred, arma::fill::value(0.01));
      arma::mat y_sim = kr.simulate(nsim, seed, X_pred, noise_pred, false);
    ...
    // Benchmark UPDATE
    {
      ...
      kr.update(y_update, noise_update, X_update);
    ...
    int seed = 123 + iter;
    arma::colvec noise_pred(n_pred, arma::fill::value(0.01));
    kr2.simulate(nsim, seed, X_pred, noise_pred, true);  // will_update=true

    {
      auto t0 = std::chrono::high_resolution_clock::now();

      arma::mat result = kr2.update_simulate(y_update, noise_update, X_update);
