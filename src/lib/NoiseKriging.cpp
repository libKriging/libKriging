    ...
    if (objective.compare("LL") == 0) {
        if (Optim::reparametrize) {
            fit_ofn = CacheFunction(
                    [this](const arma::vec &_gamma, arma::vec *grad_out, NoiseKriging::OKModel *okm_data) {
            ...
            while (retry <= Optim::max_restart) {
                ...
                double sol_to_b_sigma2
                        = Optim::reparametrize ? std::abs(gamma_tmp.at(d) - gamma_upper.at(d))
                                               : std::abs(
                                        gamma_tmp.at(d) - gamma_lower.at(d));  // just consider sigma2 upper bound
...
void NoiseKriging::save(const std::string filename) const {
    ...
    if (use_hdf5) {
        ...
        m_noise.save(arma::hdf5_name(filename, "noise", appflag));
...
NoiseKriging NoiseKriging::load(const std::string filename) {
    ...
    if (version != 1) {
        throw std::runtime_error(
                asString("Bad version to load from '", filename, "'; found ", version, ", requires 1"));
    }
    std::string content;
    ...
    kr.m_noise.load(arma::hdf5_name(filename, "noise"));
