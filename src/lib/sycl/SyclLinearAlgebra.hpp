// UNVERIFIED SYCL / Intel oneAPI port of src/lib/cuda/CudaLinearAlgebra.cuh
// -- see the CUDA header for the design rationale and
// src/lib/hip/HipLinearAlgebra.hpp for the 'this was never compiled or
// run' caveat that applies here too (no oneAPI toolchain / Intel GPU).
// Same public API as LinearAlgebraCuda / LinearAlgebraHip.
#ifndef LIBKRIGING_SRC_LIB_SYCL_SYCLLINEARALGEBRA_HPP
#define LIBKRIGING_SRC_LIB_SYCL_SYCLLINEARALGEBRA_HPP

// Only compiled/declared when the project is configured with
// -DENABLE_SYCL_ITERATIVE=ON (see root CMakeLists.txt). Kept behind this
// macro so a default (non-CUDA) build never sees CUDA types/symbols, and
// LIBKRIGING_USE_SYCL_ITERATIVE is only ever defined by CMake, never
...
// materialized on the GPU either -- see SyclLinearAlgebraKernel.cpp's rmul kernel --
...
// True iff a SYCL GPU was found at runtime (lazy-initialized, cached).
// Independent of the compile-time flag: a build compiled with SYCL support
...
// and ignored, same effective behavior as x0=0 always. No SYCL hardware was
// available to validate a real port of CudaLinearAlgebra.cpp's warm-start
// preamble when this parameter was added; that CUDA implementation is the
// reference to port.
...
#endif  // LIBKRIGING_SRC_LIB_SYCL_SYCLLINEARALGEBRA_HPP
