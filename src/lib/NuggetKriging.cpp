...
void NuggetKriging::populate_Model(KModel& m,
  ...
  const bool update_eligible = !m_is_empty && (m_sigma2 / (m_sigma2 + m_nugget) == alpha)
                               && (m_theta.size() == theta.size()) && (theta - m_theta).is_zero()
                               && (m_T.memptr() != nullptr) && (m_X.n_rows > m_T.n_rows);
  KrigingImpl::populate_Model(m, theta, alpha, KrigingImpl::ones, update_eligible, bench);
  if (!m_est_beta) {
    m.betahat = arma::vec(m_F.n_cols, arma::fill::zeros);  // whatever: not used
  }
}
...
LIBKRIGING_EXPORT std::tuple<double, arma::vec> NuggetKriging::logLikelihoodFun(const arma::vec& _theta_alpha,
                                                                                const bool _grad,
                                                                                const bool _bench) {
  return eval_objective(_theta_alpha.n_elem, _grad, _bench, [&](arma::vec* g, std::map<std::string, double>* b) {
    return _logLikelihood(_theta_alpha, g, nullptr, b);
  });
}
...
double NuggetKriging::_logMargPost(const arma::vec& _theta_alpha,
  ...
  if (grad_out != nullptr) {
    ...
    arma::vec ans = compute_lmp_theta_ans(m, _theta, _sigma2, Rinv_X_Xt_Rinv_X_inv_Xt_Rinv, Q_output, bench);
    ...
    if (m_est_sigma2 || m_est_nugget) {
      ...
      gradR_d.diag().zeros();
      Wb_k = trans(LinearAlgebra::solve_upper(m.L.t(), LinearAlgebra::solve_lower(m.L, gradR_d)))
...
LIBKRIGING_EXPORT std::tuple<double, arma::vec> NuggetKriging::logMargPostFun(const arma::vec& _theta_alpha,
                                                                              ...
                                                                              const bool _bench) {
  return eval_objective(_theta_alpha.n_elem, _grad, _bench, [&](arma::vec* g, std::map<std::string, double>* b) {
    return _logMargPost(_theta_alpha, g, nullptr, b);
  });
}
...
NuggetKriging::predict(const arma::mat& X_n, bool return_stdev, bool return_cov, bool return_deriv) {
  ...
  const double sigma2
      = m_sigma2 * (m_objective.compare("LMP") == 0 ? (n_o - m_F.n_cols) / (n_o - m_F.n_cols - 2) : 1.0);
...
LIBKRIGING_EXPORT arma::mat NuggetKriging::update_simulate(const arma::vec& y_u, const arma::mat& X_u) {
  ...
  return update_simulate_impl(y_u,
                              ...
                              /*R_uo_factor=*/alpha,
                              /*R_un_factor=*/alpha,
                              /*R_un_coincident_to_one=*/lastsim_with_nugget,
                              /*Sigma_divisor=*/alpha);
}
...
LIBKRIGING_EXPORT void NuggetKriging::update(const arma::vec& y_u, const arma::mat& X_u, const bool refit) {
  ...
  if (refit) {  // re-fit with parameter optimization
    ...
    update_no_refit_impl(
        ...
        X_u,
        /*extend_class_data=*/[] {},
        /*build_model=*/[this, alpha] { return make_Model(m_theta, alpha, nullptr); });
  }
}
...
NuggetKriging NuggetKriging::load(const std::string filename) {
  ...
  NuggetKriging kr(j["covType"].template get<std::string>());  // _Cov_pow & std::function embedded by make_Cov
