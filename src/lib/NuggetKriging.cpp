...
LIBKRIGING_EXPORT arma::mat NuggetKriging::covFun(const arma::mat& X1, const arma::mat& X2) {
  arma::mat Xn1 = X1;
  arma::mat Xn2 = X2;
...
NuggetKriging::KModel NuggetKriging::make_Model(const arma::vec& theta, const double alpha,
  ...
  bool update = (m_sigma2 / (m_sigma2 + m_nugget) == alpha) && (m_theta.size() == theta.size()) && (theta - m_theta).is_zero() && (this->m_T.memptr() != nullptr) && (n > this->m_T.n_rows);
  if (update) { 
    m.L = LinearAlgebra::update_cholCov(&(m.R), m_dX, theta, Cov, alpha, NuggetKriging::ones, m_T, m_R);
  } else 
    m.L = LinearAlgebra::cholCov(&(m.R), m_dX, theta, Cov, alpha, NuggetKriging::ones);
...
double NuggetKriging::_logLikelihood(const arma::vec& _theta_alpha,
  ...
  if (m_est_sigma2) {
    ...
    if (m_est_nugget) {
      ...
      _alpha = m_sigma2 / (m_nugget + m_sigma2); // force alpha value, and its derivative will be 0 (to force gradient convergence)
    }
  }
...
double NuggetKriging::_logMargPost(const arma::vec& _theta_alpha,
  ...
  if (grad_out != nullptr) {
    ...
    if (m_est_sigma2 || m_est_nugget) {
      ...
      gradR_d.diag().zeros();
      Wb_k = trans(
                 LinearAlgebra::solve(trans(m.L), LinearAlgebra::solve(m.L, gradR_d)))
             - gradR_d * Rinv_X_Xt_Rinv_X_inv_Xt_Rinv;
      double ans_d
          = -sum(Wb_k.diag()) / 2.0 + as_scalar(trans(m_y) * trans(Wb_k) * Q_output) / (2.0 * _sigma2);
...
LIBKRIGING_EXPORT void NuggetKriging::fit(const arma::vec& y,
  ...
  if (optim == "none") {  // just keep given theta, no optimisation of ll (but estim beta still possible)
    ...
    for (arma::uword i = 0; i < theta0.n_rows; i++) {
      ...
      NuggetKriging::KModel m = make_Model(theta0.row(i).t(), alpha0[i % alpha0.n_elem], nullptr);
...
NuggetKriging::predict(const arma::mat& X_n, bool withStd, bool withCov, bool withDeriv) {
 ...
 */
LIBKRIGING_EXPORT arma::mat NuggetKriging::simulate(const int nsim, const int seed, const arma::mat& X_n, const bool withNugget, const bool willUpdate) {
  arma::uword n_n = X_n.n_rows;
  arma::uword n_o = m_X.n_rows;
  arma::uword d = m_X.n_cols;
  if (X_n.n_cols != d)
...
arma::cout << "t(Rstar_on)*Rstar_on:" <<  trans(Rstar_on) * Rstar_on << arma::endl;
  ...
  arma::mat LSigma_nKo = LinearAlgebra::safe_chol_lower(Sigma_nKo / alpha);
...
LIBKRIGING_EXPORT arma::mat NuggetKriging::update_simulate(const arma::vec& y_u, const arma::mat& X_u) {
  ...
  double m_alpha = m_sigma2 / (m_sigma2 + m_nugget);
  double alpha = 1.0;
  if (lastsim_withNugget) alpha = m_alpha;
    ...
    for (arma::uword i = 0; i < n_u; i++) {
      for (arma::uword j = 0; j < n_n; j++) {
        ...
        if (dij.is_zero(arma::datum::eps))
          ...
          lastsimup_R_un.at(i, j) = Cov(dij, m_theta) * alpha;
      }
    }
