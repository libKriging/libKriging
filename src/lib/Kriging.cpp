...
void Kriging::populate_Model(KModel& m, const arma::vec& theta, std::map<std::string, double>* bench) const {
  ...
  // Always compute GLS beta for residual and SSE (even when beta is fixed)
  ...
  if (m_est_beta) {
    ...
    m.betahat = arma::vec(p, arma::fill::zeros);  // not used externally
  }

  // Compute residual in whitened space using GLS beta
...
double Kriging::_logLikelihood(const arma::vec& _theta,
  ...
  if (grad_out != nullptr) {
    ...
    if (hess_out != nullptr) {
      ...
      // Compute Qstar on demand for Hessian (not stored in populate_Model)
      arma::mat Qstar_h;
      arma::mat Rstar_tmp;
      LinearAlgebra::qr_econ(Qstar_h, Rstar_tmp, m.Fstar);

      // Compute Linv on demand for Hessian
      if ((m.Linv.memptr() == nullptr) || (arma::size(m.Linv) != arma::size(m.L))) {
        m.Linv = LinearAlgebra::solve_lower(m.L, arma::mat(n, n, arma::fill::eye));
      }
      t0 = Bench::toc(bench, "Qstar,Linv for Hessian", t0);
  ...
  arma::mat Rinv_X_Xt_Rinv_X_inv_Xt_Rinv
      = Rinv_X
        * (LinearAlgebra::solve_upper(
            LX.t(),
            LinearAlgebra::solve_lower(LX, Rinv_X.t())));  // compute  Rinv_X_Xt_Rinv_X_inv_Xt_Rinv through one forward
...
// Set OpenMP threads (for Armadillo operations that use OpenMP)
      ...
      auto optimize_worker = [&](arma::uword start_idx) -> OptimizationResult {
        ...
        try {
          arma::vec gamma_tmp = theta0.row(start_idx % multistart).t();
          if (Optim::reparametrize)
            gamma_tmp = Optim::reparam_to(theta0.row(start_idx % multistart).t());
          ...
          while (retry <= Optim::max_restart) {
            ...
            double sol_to_lb = Optim::reparametrize ? arma::min(arma::abs(Optim::reparam_from(gamma_tmp) - theta_lower))
                                                    : arma::min(arma::abs(gamma_tmp - theta_upper));
            double sol_to_ub = Optim::reparametrize ? arma::min(arma::abs(Optim::reparam_from(gamma_tmp) - theta_upper))
...
LIBKRIGING_EXPORT void Kriging::update(const arma::vec& y_u, const arma::mat& X_u, const bool refit) {
  ...
  if (refit) {  // Warm restart: extend data and run single BFGS from current theta
    ...
    // Set up objective function based on current objective
    std::function<double(const arma::vec&, arma::vec*, arma::mat*, Kriging::KModel*)> fit_ofn;
    if (m_objective == "LL") {
      if (Optim::reparametrize) {
        fit_ofn = [this](const arma::vec& _gamma, arma::vec* grad_out, arma::mat* hess_out, Kriging::KModel* km_data) {
          const arma::vec _theta = Optim::reparam_from(_gamma);
        ...
        fit_ofn = [this](const arma::vec& _gamma, arma::vec* grad_out, arma::mat* hess_out, Kriging::KModel* km_data) {
          double ll = this->_logLikelihood(_gamma, grad_out, hess_out, km_data, nullptr);
          if (grad_out != nullptr)
            *grad_out = -*grad_out;
          if (hess_out != nullptr)
            *hess_out = -*hess_out;
      ...
      throw std::invalid_argument("Unsupported objective in update: " + m_objective);
    }
    ...
    arma::vec gamma_upper = theta_upper;
    if (Optim::reparametrize) {
      gamma_start = Optim::reparam_to(m_theta);
      gamma_lower = Optim::reparam_to(theta_lower);
      gamma_upper = Optim::reparam_to(theta_upper);
    }
    ...
    // Run single BFGS from current theta (warm restart)
    lbfgsb::Optimizer optimizer{d};
    ...
    optimizer.minimize(
        [&km, &fit_ofn](const arma::vec& vals_inp, arma::vec& grad_out) -> double {
          return fit_ofn(vals_inp, &grad_out, nullptr, &km);
        },
        gamma_tmp,
        gamma_lower.memptr(),
        gamma_upper.memptr(),
