...
arma::colvec Kriging::ones = arma::ones<arma::vec>(0);
...
double Kriging::_logLikelihood(const arma::vec& _theta,
  ...
  if (grad_out != nullptr) {
    ...
    if (m.Linv.memptr()==nullptr) {
      m.Linv =LinearAlgebra::solve(m.L, arma::eye(n, n));
    ...
    const arma::colvec zeros = arma::zeros(d);
...
double Kriging::_leaveOneOut(const arma::vec& _theta,
  ...
  arma::colvec sigma2LOO = 1 / B.diag();
  t0 = Bench::toc(bench, "S2l = 1 / diag(Q)", t0);

  arma::colvec errorsLOO = sigma2LOO % By;
  ...
  if (grad_out != nullptr) {
    ...
    for (arma::uword i = 0; i < n; i++) {
      for (arma::uword j = 0; j < i; j++) {
        gradR.slice(i).col(j) = m->R.at(i, j) * DlnCovDtheta(m_dX.col(i * n + j), _theta);
      }
    }
    t0 = Bench::toc(bench, "gradR = R * dlnCov(dX)", t0);
    ...
    for (arma::uword k = 0; k < m_X.n_cols; k++) {
      t0 = Bench::tic();
      arma::mat gradR_k = arma::mat(n, n, arma::fill::none);
      for (arma::uword i = 0; i < n; i++) {
        gradR_k.at(i, i) = 0;
        for (arma::uword j = 0; j < i; j++) {
      ...
      arma::colvec diagdB = - LinearAlgebra::diagABA(B, gradR_k);
      t0 = Bench::toc(bench, "diagdQ = DiagABA(B, gradR_k)", t0);

      arma::colvec dsigma2LOO = -sigma2LOO % sigma2LOO % diagdB;
      t0 = Bench::toc(bench, "dS2l = -S2l % S2l % diagdQ", t0);

      arma::colvec derrorsLOO = dsigma2LOO % By - sigma2LOO % (B * (gradR_k * By));
...
//  arma::mat L = m->T;
//
//  m->M =LinearAlgebra::solve(L, X);
//  t0 = Bench::toc(bench, "M = L \\ F", t0);
//
//  arma::mat R_inv_X =LinearAlgebra::solve(trans(L), m->M);
...
//  arma::mat R_inv_X_Xt_R_inv_X_inv_Xt_R_inv
      //= R_inv_X
        //* (solve(trans(LX),
                //LinearAlgebra::solve(LX, trans(R_inv_X))));  // compute  R_inv_X_Xt_R_inv_X_inv_Xt_R_inv through one forward
//  // and one backward solve
//  t0 = Bench::toc(bench, "RiFFtRiFiFtRi = RiF * RiFt \\ M \\ Mt", t0);
//
//  arma::colvec Yt =LinearAlgebra::solve(L, m_y);
//  t0 = Bench::toc(bench, "Yt = L \\ y", t0);
//
//  if (m->is_beta_estim) {
    //arma::mat Q;
    //arma::mat G;
    //arma::qr_econ(Q, G, m->M);
    //t0 = Bench::toc(bench, "Q,G = QR(M)", t0);
    //m->beta =LinearAlgebra::solve(G, Q.t() * Yt);
    //t0 = Bench::toc(bench, "B = G \\ Yt*Qt", t0);
//  }
//
//  m->z = Yt - m->M * m->beta;  // required for later predict
//  t0 = Bench::toc(bench, "z = Yt - M * B", t0);
//
//  arma::mat yt_R_inv = trans(solve(trans(L), Yt));
...
//
//  if (m->is_sigma2_estim)  // means no sigma2 provided
    //m->sigma2 = S_2(0, 0) / (n - m_F.n_cols);
//
...
//
      //Wb_k = trans(solve(
                 //trans(L),LinearAlgebra::solve(L, gradR_k)))
...
Kriging::predict(const arma::mat& X_n, bool withStd, bool withCov, bool withDeriv) {
  ...
  arma::colvec yhat_n(n_n);
  arma::colvec ysd2_n = arma::zeros(n_n);
  arma::mat Sigma_n = arma::zeros(n_n, n_n);
  arma::mat Dyhat_n = arma::zeros(n_n, d);
  arma::mat Dysd2_n = arma::zeros(n_n, d);
...
if (withDeriv) {
  ...
  arma::mat h_eye_d = h * arma::eye(d, d);
  ...
  for (arma::uword i = 0; i < n_n; i++) {  // for each predict point... should be parallel ?
      ...
      arma::mat tXn_n_repd_i = arma::trans(Xn_n.col(i) * arma::ones(1, d));  // just duplicate X_n.row(i) d times
      ...
      if (withStd) {
        arma::mat DEcirc_n_i = LinearAlgebra::solve(Rstar_on, DF_n_i - W_i * m_M);
        Dysd2_n.row(i) = -2 * W_i.t() % Rstar_on.col(i) + 2 * Ecirc_n.t() * DEcirc_n_i;
  ...
  return std::make_tuple(std::move(yhat_n),
                         ...
                         std::move(Sigma_n),
                         std::move(m_scaleY * Dyhat_n),
                         std::move(m_scaleY * Dysd2_n/(2*arma::sqrt(ysd2_n))));
  /*if (withStd)
    if (withCov)
...
LIBKRIGING_EXPORT arma::mat Kriging::simulate(const int nsim, const int seed, const arma::mat& X_n){ //, const bool will_update) {
  ...
  arma::colvec yhat_n = F_n * m_beta + trans(Rstar_on) * m_z;
  t0 = Bench::toc(nullptr,"yhat_n        ", t0);
