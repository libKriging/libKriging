...
template <>
char const* enumStrings<OrdinaryKriging::RegressionModel>::data[] = {"constant", "linear", "quadratic"};

OrdinaryKriging::RegressionModel OrdinaryKriging::RegressionModelUtils::fromString(const std::string& s) {
  return enumFromString<RegressionModel>(s);
}

std::string OrdinaryKriging::RegressionModelUtils::toString(const OrdinaryKriging::RegressionModel& e) {
  return enumToString<RegressionModel>(e);
}
